...
function UpdateModal({ getData, modalMode, setModalMode, tempProduct, setTempProduct, isModalOpen, setIsModalOpen }) {
  ...
  const addProduct = async () => {
    try {
      const response = await axios.post(`${apiBase}/v2/api/${apiPath}/admin/product`, {
        data: {
          ...tempProduct,
  ...
  const editProduct = async () => {
    try {
      const response = await axios.put(`${apiBase}/v2/api/${apiPath}/admin/product/${tempProduct.id}`, {
        data: {
          ...tempProduct,
          origin_price: Number(tempProduct.origin_price),
          price: Number(tempProduct.price),
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      const response = await axios.post(`${apiBase}/v2/api/${apiPath}/admin/upload`, formData, {
        headers: {
